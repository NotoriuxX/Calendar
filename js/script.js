  ...
  fetch("subir_datos.php", {
    ...
    .then(text => {
      if (text === "Creado con éxito") {
        swal("Registrado exitosamente", "Hora agendada exitosamente: " + fecha + " a las " + hora + " hrs.", "success").then(() => {
          location.reload();
        });
        ...
        if (horaRegistrada.length >= 4) {
          swal("No disponible", "La hora seleccionada ya no se encuentra disponible", "error").then(() => {
            location.reload();
          });
        } else {
          swal("Registrado", "La hora seleccionada ya se encuentra registrada", "warning").then(() => {
            location.reload();
          });
        }
      } else {
    ...
    .catch(error => {
      console.error(error);
      swal("Error", "Ha ocurrido un error inesperado. Por favor, inténtelo más tarde.", "error").then(() => {
        location.reload();
      });
    });
});
